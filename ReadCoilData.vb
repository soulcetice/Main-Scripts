	...
	For i = 1 To 25 Step 1
		If  HMIRuntime.Tags("SMS_TCM::MM" & MM(i) & "_HmiPieceDspGenDat" & No(i) & "_PieceStatInf").Read <> 0 Then
			...
			Else
				...
				groupWrite("iWeightSkid_"      & group("SMS_TCM::MM" & MM(i) & "_HmiPieceDspAddDat" & No(i) & "_TrPos"pa).Value).Value = groupRead("SMS_TCM::MM" & MM(i) & "_HmiPieceDspGenDat" & No(i) & "_CoilWght").Value
...
j=1	
	For i = 1 To 25 Step 1
		If  HMIRuntime.Tags("iWidthSkid_" & i).Read <> 0 Then
			...
			groupWrite("iCoilIdTable_" & i).Value 			= groupRead("iCoilIdSkid_" & i).Value
			groupWrite("iCoilOutIdTable_" & i).Value 		= groupRead("iCoilOutIdSkid_" & i).Value
			groupWrite("iAlloyTable_"  & i).Value 			= groupRead("iAlloySkid_" & i).Value
			groupWrite("iInnDiaTable_" & i).Value 			= groupRead("iInnDiaSkid_" & i).Value
			groupWrite("iOutDiaTable_" & i).Value 			= groupRead("iOutDiaSkid_" & i).Value
			groupWrite("iLenTable_"    & i).Value 			= groupRead("iLenSkid_" & i).Value
			groupWrite("iThickTable_"  & i).Value 			= groupRead("iThickSkid_" & i).Value
			groupWrite("iWeightTable_" & i).Value 			= groupRead("iWeightSkid_" & i).Value
			groupWrite("iWidthTable_"  & i).Value 			= groupRead("iWidthSkid_" & i).Value
			groupWrite("iPieceTable_"  & i).Value 			= groupRead("iPieceSkid_" & i).Value
			groupWrite("iTrPosTable_"  & i).Value 			= groupRead("iTrPosSkid_" & i).Value
...
'Reset the data on the internal variables when the Coil leaves the Table
	For i = 1 To 25 Step 1
		If Table(i) = 0 Then		
...
'Do all writing actions at once
groupWrite.Write
